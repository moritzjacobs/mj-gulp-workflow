	...
	describe("Prepare testing: \n", () => {
		it("Delete previously compiled files ...", wait => {
			...
			describe("Running gulp tasks: \n", () =>
				new Promise(resolve => {
					require(`${__dirname}/../tasks/css/task.js`)(
						gulp,
						testConfig.css,
						testConfig.paths.css
					);
					gulp.start("css");
					...
					.then(() => {
						require(`${__dirname}/../tasks/js/task.js`)(
							gulp,
							testConfig.js,
							testConfig.paths.js
						);
					...
					.then(() => {
						require(`${__dirname}/../tasks/images/task.js`)(
							gulp,
							testConfig.images,
							testConfig.paths.images
						);
						gulp.start("images");
					})
					.then(() => {
						require(`${__dirname}/../tasks/copy/task.js`)(
							gulp,
							testConfig.copy,
							testConfig.paths.copy
						);
						gulp.start("copy");
					}));
		});
...
function runTest() {
	describe("Testing gulp tasks: \n", () => {
		...
		const compareFiles = task => {
			...
			if (
				task == "copy" &&
				copyInput.toString() === fs.readFileSync(fileCopy).toString()
			) {
				return true;
			}
