...
const configFactory = require('./lib/config.js'); // define propertiess


const properties = {}; // process cli args

properties.env = argv.env;
console.log(`Environment: ${properties.env}`);

const workflow = gulp => {
  if (!gulp) return false; // define more propertiess

  properties.moduleRootDir = __dirname;
  properties.appRootDir = AppRootDir.get();
  properties.runFrom = properties.appRootDir;
  const config = configFactory(properties); // Init all tasks
  ...
  for (const taskName in config.combinedTasks) {
    gulp.task(taskName, cb => {
      runSequence.apply(void 0, config.combinedTasks[taskName], cb);
    });
  } // special watch task
