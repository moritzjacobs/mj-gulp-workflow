...
module.exports = global => {
  if (!compileConfigExistsInAppRootDir(global) && !compileConfigExistsInCwd(global)) {
    const defaultConfig = fs.readFileSync(path.join(global.moduleRootDir, '..', 'gulp-config-default.js'), 'UTF-8'); // Write the default compileConfig file
    ...
    console.info(`Loading config from: ${global.appRootDir}/gulp-config.js`);
    return require(`${global.appRootDir}/gulp-config.js`);
