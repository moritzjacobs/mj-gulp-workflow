...
module.exports = (config, dest) => through.obj((vinylFile, encoding, callback) => {
	const outputPath = path.join(process.cwd(), dest);
	const transformedFile = vinylFile.clone();

	const options = faviconsOptions(config.iconsPath, config.themeColor, config.appName);
	...
	rfg.generateFavicon(request, path.join(outputPath, faviconSlug), (err, result) => {
		if (err) {
			throw err;
		}

		fs.writeFile(path.join(outputPath, faviconSlug, 'favicon.json'), JSON.stringify(result, null, 4), err => {
			if (err) {
				throw err;
			} else {
